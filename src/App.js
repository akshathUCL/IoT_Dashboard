...
function App() {
  ...
  const pushManualOverride = async (ad) => {
  await fetch("https://dj7r6jv7tk.execute-api.eu-north-1.amazonaws.com/override", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      ad_id: ad.ad_id,
      asset_url: ad.asset_url,
      duration_sec: ad.duration_sec ?? 10
    })
  });
  setOverrideAd(ad);
  ...
  return (
    ...
    <div className="dashboard">
      ...
      <div className="mainGrid">
        ...
        <div className="card manualAdControl">
          <h2>Manual Ad Override</h2>

          <select
            defaultValue=""
            onChange={(e) => {
              const selected = ads.find(a => a.ad_id === e.target.value);
              if (selected) pushManualOverride(selected);
            }}
          >
            <option value="" disabled>Select an ad to push...</option>
            {ads.map(ad => (
              <option key={ad.ad_id} value={ad.ad_id}>
                {ad.ad_id} (Priority: {ad.priority ?? "-"})
              </option>
            ))}
          </select>
          ...
          {manualOverride && overrideAd && (
            <p style={{ color: "orange" }}>
              ⚠️ Manual override active: <strong>{overrideAd.ad_id}</strong>
            </p>
          )}
        </div>
